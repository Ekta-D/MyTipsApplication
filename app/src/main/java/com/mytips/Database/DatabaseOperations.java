...
//        String selection[] = new String[]{"1"};
    ...
    public ArrayList<AddDay> fetchAddDayDaysAlldetails(int selected_profile, ArrayList<Profiles> profiles,String profileName) {
        ArrayList<AddDay> addDays = null;
        AddDay addDay = null;
        Cursor cursor = null;
        String query = "";
        int profileID = 0;
        ArrayList<AddDay> merged_days = new ArrayList<>();
        if (selected_profile == 0 || profileName.equalsIgnoreCase("All")) {
            ...
            addDays = new ArrayList<>();
            query = "select * from  add_table where  profile_id=  '" + selected_profile + "'  Order By start_shift_long";
            try {
                cursor = db.rawQuery(query, null);
            } catch (Exception e) {
                e.printStackTrace();
