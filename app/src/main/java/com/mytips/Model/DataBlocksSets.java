package com.mytips.Model;

/**
 * Created by Beesolver on 06-03-2018.
 */

public class DataBlocksSets {
    String mProfile;
    String mLiveTips;
    String mTournamentDowns;
    String mTipOut;
    String mHrlyWage;
    String mTipPerDown;
    String mTotalIncome;
    String mTotalTDDowns;
    String mDates;
    String mTipoutPercentage;

    public long getmEndDateLong() {
        return mEndDateLong;
    }

    public void setmEndDateLong(long mEndDateLong) {
        this.mEndDateLong = mEndDateLong;
    }

    long mEndDateLong;

    public String getmDates() {
        return mDates;
    }

    public void setmDates(String mDates) {
        this.mDates = mDates;
    }



    public String getmTipoutPercentage() {
        return mTipoutPercentage;
    }

    public void setmTipoutPercentage(String mTipoutPercentage) {
        this.mTipoutPercentage = mTipoutPercentage;
    }



    public String getmProfile() {
        return mProfile;
    }

    public void setmProfile(String mProfile) {
        this.mProfile = mProfile;
    }

    public String getmLiveTips() {
        return mLiveTips;
    }

    public void setmLiveTips(String mLiveTips) {
        this.mLiveTips = mLiveTips;
    }

    public String getmTournamentDowns() {
        return mTournamentDowns;
    }

    public void setmTournamentDowns(String mTournamentDowns) {
        this.mTournamentDowns = mTournamentDowns;
    }

    public String getmTipOut() {
        return mTipOut;
    }

    public void setmTipOut(String mTipOut) {
        this.mTipOut = mTipOut;
    }

    public String getmHrlyWage() {
        return mHrlyWage;
    }

    public void setmHrlyWage(String mHrlyWage) {
        this.mHrlyWage = mHrlyWage;
    }

    public String getmTipPerDown() {
        return mTipPerDown;
    }

    public void setmTipPerDown(String mTipPerDown) {
        this.mTipPerDown = mTipPerDown;
    }

    public String getmTotalIncome() {
        return mTotalIncome;
    }

    public void setmTotalIncome(String mTotalIncome) {
        this.mTotalIncome = mTotalIncome;
    }

    public String getmTotalTDDowns() {
        return mTotalTDDowns;
    }

    public void setmTotalTDDowns(String mTotalTDDowns) {
        this.mTotalTDDowns = mTotalTDDowns;
    }


}
